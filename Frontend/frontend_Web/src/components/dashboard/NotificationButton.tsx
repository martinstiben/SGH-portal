"use client";

interface NotificationButtonProps {
  onClick: () => void;
  unreadCount?: number;
}

export default function NotificationButton({ onClick, unreadCount = 0 }: NotificationButtonProps) {
  const hasUnreadNotifications = unreadCount > 0;

  return (
    <button
      onClick={onClick}
      className="relative p-2 rounded-lg hover:bg-gray-100 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
      aria-label={`Notificaciones${hasUnreadNotifications ? ` (${unreadCount} no leídas)` : ''}`}
    >
      {/* Ícono de campanita */}
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill={hasUnreadNotifications ? "currentColor" : "none"}
        viewBox="0 0 24 24"
        strokeWidth={1.5}
        stroke="currentColor"
        className={`w-6 h-6 transition-colors duration-200 ${
          hasUnreadNotifications ? "text-blue-600" : "text-gray-700"
        }`}
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"
        />
      </svg>
      
      {/* Indicador de notificaciones no leídas */}
      {hasUnreadNotifications && (
        <span className="absolute -top-1 -right-1 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-red-500 rounded-full">
          {unreadCount > 99 ? '99+' : unreadCount}
        </span>
      )}
    </button>
  );
}